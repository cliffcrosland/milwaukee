M.provide("facebook", new function () {
   ...
   * @param cb
   */
  that.deleteAppRequests = function (compoundRequestIds, access_token, cb) {
    if (!compoundRequestIds) return t("compoundRequestIds invalid");
    if (!FB) return t("FB not initialized");

    var responses = [],
      numIds = compoundRequestIds.length;

    for (var i = 0, len = numIds; i < len; ++i) {
      var compoundId = compoundRequestIds[i];
      FB.api(compoundRequestIds[i], {access_token:access_token},
        'delete', function (response) {
          responses.push({compoundId:compoundId, response:response });
          if (responses.length === numIds && cb) cb(responses);
        });
    }
  };
  ...
  that.sendAppRequests = function (options) {
    ...
    if (options['data']) fbUiHash.data = options['data'];
    ...
    FB.ui(fbUiHash, function (request) {
      ...
      if (!request || !request.to) {
        ...
        // Else, the requests sent successfully.
        request.processed = processed;
      ...
      if (remainingFacebookUserIds && remainingFacebookUserIds.length > 0) {
        ...
        // Done. Call the 'finished' callback.
