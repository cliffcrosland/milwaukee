M.provide("facebook", new function () {
  ...
  that.sendAppRequests = function (options) {
    var facebookUserIds = options['facebookUserIds'],
      ...
      numRequestsSuccessfullySent = options['numRequestsSuccessfullySent'] || 0,
    ...
    FB.ui(fbUiHash, function (request) {
      ...
      if (!request || !request.to) {
        ...
        request.processed = processed;
        numRequestsSuccessfullySent += request.to.length;
      ...
      if (remainingFacebookUserIds && remainingFacebookUserIds.length > 0) {
        ...
        options['numRequestsSuccessfullySent'] = numRequestsSuccessfullySent;
        that.sendAppRequests(options);
      } else {
        // Done. Call the 'finished' callback.
        request['numRequestsSuccessfullySent'] = numRequestsSuccessfullySent;
